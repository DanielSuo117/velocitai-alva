...
| `auto` | 同 `on`，并在规则交白卷时让模型推理（需要 API key；没有 key 时等同 `on`） |

**任何档位都不在运行期改写源码**：自愈只在运行期临时替换定位符，页面对象文件保持原样；修复记录写进
`reports/self-heal/proposals.jsonl`，写回源码是 [selector-self-heal](./.claude/skills/selector-self-heal/) skill 的
职责 —— 人工复核提案后另行改文件，并同样过落库闸门（规则 P0.4：运行期修好 ≠ 已入库）。没有 API key 时
`auto` 不报错、不走网络，等同 `on`。
...
- 上游 `conftest.py` 的自愈开关若给 `auto` 打开写回（`self_heal_patch`），不要跟进：本项目的
  `--self-heal=auto` 只开模型兜底，任何档位都不改写源码（规则 P0.4），写回走 selector-self-heal skill。
